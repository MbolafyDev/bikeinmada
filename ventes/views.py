...
def build_commandes_context(request):
    ...
    # Pagination
    paginator = Paginator(commandes, 18)
...
def encaissement_ventes_groupes(request):
    if request.method == 'POST':
        ...
        if not paiement:
            ...
            return redirect('cencaissement_ventes')

        paiement = get_object_or_404(Caisse, pk=paiement)
        ...
        messages.success(request, f"{commandes.count()} vente(s) encaissée(s) avec succès. Statut vente mis à jour.")
        return redirect('encaissement_ventes')
